...
class Plugin
{
    ...
    // Ex: /www/headless.php/bl-content/plugins/sitemap/db.php
    public $filenameDb;

    // (string) Absolute metadata filename and path
    // Ex: /www/headless.php/bl-plugins/sitemap/metadata.json
    ...
    {
        return '<link rel="stylesheet" type="text/css" href="' . $this->domainPath() . 'css/' . $filename . '?version=' . HEADLESS_PHP_VERSION . '">' . PHP_EOL;
    }

    public function includeJS($filename)
    {
        return '<script charset="utf-8" src="' . $this->domainPath() . 'js/' . $filename . '?version=' . HEADLESS_PHP_VERSION . '"></script>' . PHP_EOL;
    }
    ...
    {
        $bluditRoot = explode('.', HEADLESS_PHP_VERSION);
    ...
    // Example: https://byuwur.co/api/foo/bar
