<script charset="utf-8">
    class bluditAjax {
        ...
        userLogged(callBack) {
            ...
            console.log("[INFO] [HEADLESS_PHP AJAX] [userLogged()] Checking if the user is logged.");
            ...
            ajaxRequest.done(function(response, textStatus, jqXHR) {
                console.log("[INFO] [HEADLESS_PHP AJAX] [userLogged()] The user is logged.");
            });

            ajaxRequest.fail(function(jqXHR, textStatus, errorThrown) {
                // The fail is produced by admin.php when the user is not logged the ajax request is not possible and returns 401
                console.log("[INFO] [HEADLESS_PHP AJAX] [userLogged()] The user is NOT logged.");
                if (jqXHR.status == 401) {
                    callBack("You are not logged in anymore, so we can't save your settings and content.");
                }
            });
        }
        ...
        generateSlug(text, parentKey, currentKey, callBack) {
            ...
            ajaxRequest.done(function(response, textStatus, jqXHR) {
                console.log("AJAX: generateSlug(): done handler");
                callBack.val(response["slug"]);
            });

            ajaxRequest.fail(function(jqXHR, textStatus, errorThrown) {
                console.log("AJAX: generateSlug(): fail handler");
            });

            ajaxRequest.always(function() {
                console.log("AJAX: generateSlug(): always handler");
            });
        }
    }
</script>