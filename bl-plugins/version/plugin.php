...
class pluginVersion extends Plugin
{
    ...
    public function adminSidebar()
    {
        ...
        if ($this->getValue('showCurrentVersion')) {
            $html = '<a id="current-version" class="nav-link" href="' . HTML_PATH_ADMIN_ROOT . 'about' . '">' . $L->get('Version') . ' ' . (defined('HEADLESS_PHP_PRO') ? '<span class="bi-heart" style="color: #ffc107"></span>' : '') . '<span class="badge bg-warning rounded-pill">' . HEADLESS_PHP_VERSION . '</span></a>';
        }
